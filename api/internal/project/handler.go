...
// Mount registers the projects CRUD routes under /api/v1.
...
// ErrInUse case: project deletes are ON DELETE SET NULL, never restricted.
func (h *Handler) fail(w http.ResponseWriter, err error) {
	switch {
	case errors.Is(err, ErrEmptyName), errors.Is(err, ErrNameTooLong):
		httpx.WriteErr(w, http.StatusBadRequest, err.Error())
	case errors.Is(err, store.ErrNotFound):
		httpx.WriteErr(w, http.StatusNotFound, "project not found")
	case errors.Is(err, store.ErrConflict):
		httpx.WriteErr(w, http.StatusConflict, "project name already exists")
	default:
		h.log.Error("projects: unexpected error", "error", err)
